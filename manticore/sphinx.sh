...
source main
{
  type     = mysql
  sql_host = ${DB_HOST}
  sql_user = ${DB_SPHINX_USER}
  sql_pass = ${DB_SPHINX_PASS}
  sql_db   = ${DB_ASAGI_NAME}
  sql_port = ${DB_PORT}
  ...
  sql_query      = \\
      SELECT doc_id, 1 AS board, timestamp AS timestamped, thread_num AS tnum, num, subnum, name, trip, email, title, comment, \\
  ...
  sql_attr_bool = is_op
  sql_attr_timestamp = timestamped
  sql_attr_json = exif
...
for BOARD in $(echo "SELECT id, shortname FROM ${DB_FOOLFUUKA_NAME}.ff_boards WHERE sphinx = 1;" | mysql --host=${DB_HOST} --user=${DB_SPHINX_USER} --password=${DB_SPHINX_PASS} --silent --skip-column-names ${DB_FOOLFUUKA_NAME} | tr '\t+' ':'); do
cat << EOF
# /${BOARD#*:}/
source ${BOARD#*:}_main : main
{
  sql_query      = \\
      SELECT doc_id, ${BOARD%:*} AS board, timestamp AS timestamped, thread_num AS tnum, num, subnum, name, trip, email, title, comment, \\
        media_id AS mid, media_filename, media_hash, poster_ip AS pip, poster_hash AS pid, poster_country AS country, ASCII(capcode) AS cap, \\
        (subnum != 0) AS is_internal, spoiler AS is_spoiler, deleted AS is_deleted, sticky AS is_sticky, op AS is_op, \\
        (media_filename != '' AND media_filename IS NOT NULL) AS has_image, exif, media_w, media_h, media_orig \\
      FROM \`asagi\`.\`${BOARD#*:}\` WHERE doc_id >= \$start AND doc_id <= \$end

  sql_query_range      = SELECT (SELECT val FROM \`index_counters\` WHERE id = 'max_ancient_id_${BOARD#*:}'), (SELECT MAX(doc_id) FROM \`asagi\`.\`${BOARD#*:}\`)
  sql_query_post_index = REPLACE INTO \`index_counters\` (id, val) VALUES ('max_indexed_id_${BOARD#*:}', \$maxid)
}

source ${BOARD#*:}_ancient : ${BOARD#*:}_main
{
  sql_query_range      = SELECT MIN(doc_id), MAX(doc_id) FROM \`asagi\`.\`${BOARD#*:}\`
...
{
  sql_query_range      = SELECT (SELECT val FROM \`index_counters\` WHERE id = 'max_indexed_id_${BOARD#*:}'), (SELECT MAX(doc_id) FROM \`asagi\`.\`${BOARD#*:}\`)
  sql_query_post_index =
}
...
for BOARD in $(echo "SELECT shortname FROM ${DB_FOOLFUUKA_NAME}.ff_boards WHERE sphinx = 1;" | mysql --host=${DB_HOST} --user=${DB_SPHINX_USER} --password=${DB_SPHINX_PASS} --silent --skip-column-names ${DB_FOOLFUUKA_NAME}); do
cat << EOF
# /${BOARD}/
index ${BOARD}_main : main
{
  source = ${BOARD}_main
  path   = /var/lib/manticore/${BOARD}_main
}

index ${BOARD}_ancient : ${BOARD}_main
{
  source = ${BOARD}_ancient
  path   = /var/lib/manticore/${BOARD}_ancient
}

index ${BOARD}_delta : ${BOARD}_main
{
  source = ${BOARD}_delta
  path   = /var/lib/manticore/${BOARD}_delta
}
...
searchd
{
  ...
  query_log    = /var/log/manticore/sphinx-query.log
  ...
  #
  # crash_log_path    = /var/lib/manticore/log/crash
