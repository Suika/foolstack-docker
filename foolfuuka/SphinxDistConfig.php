...
source template
{
  ...
  sql_query      = \
      SELECT doc_id, <?= $example->id ?> AS board, timestamp AS timestamped, thread_num AS tnum, num, subnum, name, trip, email, title, comment, \
...
{
  sql_query      = \
      SELECT doc_id, <?= $board->id ?> AS board, timestamp AS timestamped, thread_num AS tnum, num, subnum, name, trip, email, title, comment, \
        media_id AS mid, media_filename, media_hash, poster_ip AS pip, poster_hash AS pid, poster_country AS country, ASCII(capcode) AS cap, \
        (subnum != 0) AS is_internal, spoiler AS is_spoiler, deleted AS is_deleted, sticky AS is_sticky, op AS is_op, \
        (media_filename != '' AND media_filename IS NOT NULL) AS has_image, exif, media_w, media_h, media_orig \
...
index template
{
  type         = plain
  source       = template
  path         = <?= rtrim($sphinx['working_directory'], '/') ?>/main
  morphology   = none
...
index <?= $board->shortname.'_ancient'.$i ?> : template
{
  ...
  path   = <?=rtrim($sphinx['working_directory'], '/') ?>/<?= $board->shortname ?>_ancient<?= $i.PHP_EOL ?>
}
<?php endfor; ?>
...
index <?= $board->shortname.'_main'.$i ?> : <?= $board->shortname.'_ancient'.$i.PHP_EOL ?>
{
  ...
  path   = <?=rtrim($sphinx['working_directory'], '/') ?>/<?= $board->shortname ?>_main<?= $i.PHP_EOL ?>
}
<?php endfor; ?>
...
index <?= $board->shortname.'_delta' ?> : <?= $board->shortname.'_ancient'.PHP_EOL ?>
{
  ...
  path   = <?=rtrim($sphinx['working_directory'], '/') ?>/<?= $board->shortname ?>_delta
}
...
searchd
{
  ...
  binlog_path    = <?=rtrim($sphinx['working_directory'], '/') ?>
